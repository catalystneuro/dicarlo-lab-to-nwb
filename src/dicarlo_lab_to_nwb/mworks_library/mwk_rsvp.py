...
from dicarlo_lab_to_nwb.conversion.intananalogueventsinterface import (
    IntanAnalogEventsInterface,
)
from dicarlo_lab_to_nwb.mworks_library.mwk2reader import MWKFile

# The photodiode is wired to the first ADC input of the Intan board
PHOTODIODE_CHANNEL_NAME = "ANALOG-IN-1"
...
def dump_events_rsvp(SAMPLING_FREQUENCY_HZ, filename, intan_file_path, digi_event_filepath, output_dir: str = "./"):
    ...
    ###########################################################################
    # The photodiode is an Intan ADC channel, read through neuroconv's events interface. The cut sits midway
    # between the levels of the line, where the quartile midpoint this used to take fell inside the baseline
    # noise and fired on it: the screen change was reported about 16 ms too early
    analog_events_interface = IntanAnalogEventsInterface(
        file_path=intan_file_path,
        detection_configuration={
            PHOTODIODE_CHANNEL_NAME: [{"signal_conditioning": {"binarize": "midpoint"}, "detection": "rising"}]
        },
    )
    (photodiode_events,) = analog_events_interface._get_events_data_dict().values()
    v_on = np.round(np.asarray(photodiode_events.timestamps) * SAMPLING_FREQUENCY_HZ).astype("int64")

    # The screen changes within one stimulus of its pulse, so the search window is the stimulus duration.
    # It is in samples, as `samp_on` is
    window_in_samples = int(output["stim_on_time_ms"] * SAMPLING_FREQUENCY_HZ / 1000.0)
    first_edge_in_window = np.searchsorted(v_on, samp_on)
    has_edge = (first_edge_in_window < len(v_on)) & (
        v_on[np.minimum(first_edge_in_window, len(v_on) - 1)] < samp_on + window_in_samples
    )
    if not has_edge.all():
        raise ValueError(
            f"{int((~has_edge).sum())} of {len(samp_on)} sample-on pulses have no photodiode rising edge within "
            f"{output['stim_on_time_ms']:.0f} ms, so the photodiode is not tracking the stimulus in this session."
        )
    photodiode_on = v_on[first_edge_in_window]

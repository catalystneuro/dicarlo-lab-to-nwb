...
class MWK2Reader:
    ...
    def __init__(self, filename):
        ...
        raw_codecs = [data for (_, _, data) in self.events([ReservedEventCode.RESERVED_CODEC_CODE])]
        if raw_codecs:
            assert all(map((lambda rc: rc == raw_codecs[0]), raw_codecs[1:])), "MWK2 file contains conflicting codecs"
            self.codec = dict((code, props["tagname"]) for code, props in raw_codecs[0].items())
            self.reverse_codec = dict((tagname, code) for code, tagname in self.codec.items())
    ...
    def __getattr__(self, name):
        ...
        if name == "num_events":
            ...
            value = self._conn.execute("SELECT min(time), max(time) " "FROM events").fetchone()
        else:
...
class MWKFile:
    ...
    def get_events_iter(self, codes=(), time_range=(None, None)):
        ...
        if (min_time is not None) and (max_time is not None) and (min_time > max_time):
            raise RuntimeError("Minimum event time must be less than or equal " "to maximum event time")

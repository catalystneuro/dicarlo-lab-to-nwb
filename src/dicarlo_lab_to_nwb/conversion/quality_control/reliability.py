...
def get_p_values(rates, num_permutations: int = 100) -> np.ndarray:
    ...
    for u in range(n_units):
        ...
        for _ in range(num_permutations):
            ...
            signal_var_null = np.var(np.nanmean(rates_u_null, axis=-1), ddof=1) - np.nanvar(rates_u_null) / (
                n_reps * n_stimuli
            )
            sv_null_dist.append(signal_var_null)

            # adjusted variance of the mean (subtracting sampliing error)
            signal_var_test = np.var(np.nanmean(rates_u_resampled, axis=-1), ddof=1) - np.nanvar(rates_u_resampled) / (
                n_reps
            )
    ...
    for nwb_filepath in nwb_filepaths:
        with NWBHDF5IO(nwb_filepath, mode=mode) as io:
            ...
            for i in range(n_units):
                ...
                psth_unit = psth[i]
                rates[i, :, :] = (
                    np.sum(psth_unit[..., intg_window_idx[0] : intg_window_idx[1]], axis=-1) / intg_window_size_s
                )

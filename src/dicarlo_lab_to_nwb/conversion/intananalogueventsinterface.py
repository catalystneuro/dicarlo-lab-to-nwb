"""Vendored copy of neuroconv's `IntanAnalogEventsInterface`, which is not released yet.

The body below is the file from neuroconv PR 2052 at commit 41b9cb10eb164daaf610aac756be9a91dcca6556, with
only its relative imports rewritten as absolute ones. Everything it imports is already in neuroconv 0.10.2,
so this module is the interface class and nothing else. Delete it and import the interface from
`neuroconv.datainterfaces` once the PR is released, which should leave behaviour unchanged.

https://github.com/catalystneuro/neuroconv/blob/41b9cb10eb164daaf610aac756be9a91dcca6556/src/neuroconv/datainterfaces/ecephys/intan/intananalogueventsinterface.py
"""
...
from neuroconv.datainterfaces.ecephys.intan._utils import _warn_if_split_siblings_detected
from neuroconv.datainterfaces.events.baseeventsinterface import BaseEventsInterface, _EventsData
from neuroconv.tools.events import _get_event_type_source_ids, _resolve_detection_plan, _validate_detection_configuration
from neuroconv.tools.signal_processing import _condition_signal, _detect_events, _frames_to_seconds
